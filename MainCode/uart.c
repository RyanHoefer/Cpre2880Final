...
void uart_init(void){
  ...
  //enable UART1 Rx and Tx on port B pins after clearing other alternate functions
  GPIO_PORTB_PCTL_R &= ~0x000000FF; 
  GPIO_PORTB_PCTL_R |= 0x00000011;

  //calculate baud rate
  uint16_t iBRD = 8; //16000000 / (16 * 115200) = 8.680556
  uint16_t fBRD = 44; //(.680556 * 64) + 0.5 = 44

  //turn off UART1 while setting it up
  UART1_CTL_R &= ~0x00000001;
  //UART1_CTL_R &= ~0x00000001; //page 918
  ...
  //note from the datasheet UARTCCC regilkjster description:
  ...
  UART1_CTL_R |= 0x00000301; //page 918
}
...
void uart_sendChar(char data){
  ...
  while ((UART1_FR_R & 0x20) != 0) {}; //wait until the UART transmitter is not full

  UART1_DR_R = data; //write the char to UART
}

char uart_receive(void){
	
  while ((UART1_FR_R & 0x10) != 0) {}; //wait until UART receiver is not empty

  return (char)(UART1_DR_R & 0xFF); //read the char from UART
}
